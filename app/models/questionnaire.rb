...
# Tienen que estar definidos también, aunque de otra forma en el modelo page.rb
# P.ej "Bases de datos"
# P.ej "Nº trabajadores, Nivel, Modalidad"
# Toda la información a cerca de nº de trabajadores, nivel y modalidad
# Títulos de la primera columna, P.ej, AJAX, Oracle, ...

	has_many :rows_title, :through => :pages, :class_name => 'Question', 
	:order => "pages.position, questions.position", 
	:conditions => "type in ('Questions::FormaHeadingTitle')", :include => :page

	has_many :rows_title_data, :through => :pages, :class_name => 'Question', 
	:order => "pages.position, questions.position", 
	:conditions => "type in ('Questions::FormaHeading')", :include => :page

	has_many :rows_data, :through => :pages, :class_name => 'Question', 
	:order => "pages.position, questions.position", 
	:conditions => "type in #{Question.types_for_sql(Question.forma_data_types)}", :include => :page   

  	has_many :rows_data_trab, :through => :pages, :class_name => 'Question', 
  	:order => "pages.position, questions.position", 
  	:conditions => "type in ('Questions::FormaTextField')", :include => :page   
  
  	has_many :rows_data_niv, :through => :pages, :class_name => 'Question', 
  	:order => "pages.position, questions.position", 
  	:conditions => "type in ('Questions::FormaDropDownFieldNivel')", :include => :page   
  
  	has_many :rows_data_mod, :through => :pages, :class_name => 'Question', 
  	:order => "pages.position, questions.position", 
  	:conditions => "type in ('Questions::FormaDropDownFieldMod')", :include => :page       
    
  	has_many :title_courses, :through => :pages, :class_name => 'Question', 
  	:order => "pages.position, questions.position", 
  	:conditions => "type in ('Questions::FormaHeadingTitle', 'Questions::FormaLabel')", :include => :page   

  	has_many :forma_data, :through => :pages, :class_name => 'Question', 
  	:order => "pages.position, questions.position", 
  	:conditions => "type in ('Questions::FormaDataTextField')", :include => :page   
